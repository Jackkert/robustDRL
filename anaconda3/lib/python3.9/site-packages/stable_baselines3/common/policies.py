...
class BasePolicy(BaseModel):
    ...
    @abstractmethod
    def _predict(self, observation: th.Tensor, costs, deterministic: bool = False) -> th.Tensor:
        """
    ...
    def predict(
        ...
        """
        def attack(costs, observation, epsilon):
            # print("costs grad_fn:",costs.grad_fn)
            # print(costs.grad)
            # print("observation grad_fn:",observation.grad_fn)
            # print(observation.sum())
            return observation
            if(observation.grad_fn == None):
                return observation

            data_grad = costs.backward(observation,retain_graph=True)

            # data_grad = th.autograd.grad(costs, observation)
            print("data_grad:", data_grad)
            try:
                
                sign_data_grad = data_grad[0].sign()
                
                pertu_observation = observation + epsilon * sign_data_grad
                # print(pertu_observation)
                return pertu_observation
        ...
        # try:
        #     list_size = list(observation.shape)
        #     rand_torch = np.random.rand(list_size[0],list_size[1]) * 10
        #     observation = np.add(rand_torch, observation)
        # except Exception as e: 
        #     print("Exception:",e)

        # observation, vectorized_env = self.obs_to_tensor(observation)
        # print(observation)
        # observation = attack(costs, observation, 0.005)
        ...
        try:
            action_torch = th.Tensor(actions[0])
            costs = 0.5 * th.sum(th.square(action_torch))
            for i,j in enumerate(observation):
                for g,h in enumerate(j):
                    costs.backward(h, retain_graph=True)
                    
            epsilon = 0.0
            pertu_observation = observation + epsilon * observation.grad.sign()
            actions = self._predict(pertu_observation, deterministic=deterministic)
            print("Succesfull backward()")
        except:
            print("Failed backward()")
        # print("observation.grad=",observation.grad)
        # Convert to numpy
